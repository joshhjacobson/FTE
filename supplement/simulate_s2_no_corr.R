...
build_ensemble <- function(arr_dat, index, rng, rhored,
                           xi=0, smooth=c(1.5, 1.5, 1.5), var=c(1, 1), n=11) {
  ...
  ## NOTE: xi is set as 0, rhored is now adjusted above s.t. (true_rho = xi) holds
  ...
  nam <- paste("exceed_dat_no_corr_s", rng[1], sep="")
...
range_sim <- function(s_1, N) {
  ...
  s_2 <- c(0.5*s_1, 0.9*s_1, s_1, 1.1*s_1, 1.5*s_1)
  ...
  check <- try(load(paste("exceed_dat_no_corr_s", s_1, ".RData", sep="")))
...
# NOTE: xi = 0 (where xi is the weighting / skill parameter)
s_1 <- 2
N <- 5000
